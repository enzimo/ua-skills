...
description: Apply this skill when debugging issues. Four-phase debugging methodology with root cause analysis. Use when investigating bugs, fixing test failures, or troubleshooting unexpected behavior. Emphasizes NO FIXES WITHOUT ROOT CAUSE FIRST.
---
...
When gathering command evidence in Universal Agents, load
`shell-execution-workflows` before calling `shell`. Pass the underlying command
without an `rtk` prefix, use the narrowest `work_dir` and explicit timeout, and
inspect separate output/error, exit code, termination reason, RTK mode, and
warnings. Preserve useful partial output from timeouts or cancellations instead
of treating every non-success as the same failure. Use `parallel=true` only for
independent probes; disable it when investigating ordering, races, or shared
state.
...
- [ ] Fix addresses root cause, not symptoms (see `root-cause-tracing.md`)
